...
async function writeOutput() {
  ...
  usdValueWETH = (currentValueUSD_tmp2 + amount0USD + feeAmount0USD).toFixed(0);
  usdValueUSDC = (currentValueUSD_tmp3 + amount1USD + feeAmount1USD).toFixed(0);
...
let exportUsdValueAll3 = 0;

async function waitResult_short3(callback) {
  await readValue();
  exportUsdValueAll3 = usdValueAll; // Update the global variable with the correct value
  callback(exportUsdValueAll3); // Call the callback function with someVariable as argument
}
